                        ...
                        if (ca.tableIndex > -1 && response.toUpper().contains("ERROR") && !m_settings->ignoreErrors()) {
                            ...
                            if (!holding) {
                                ...
                                if (result == QMessageBox::Ignore) {
                                    ...
                                    m_serialPort.write("~"); // FIXME: Reset GRBL Here
...
int frmMain::sendCommand(QString command, int tableIndex, bool showInConsole, bool wait)
{
    ...
    foreach (QScriptEngine *en, m_plugins) {
        QScriptValue result = en->evaluate(QString("preprocessCommand('%1')").arg(command.replace("'", "\'")));
        if (!result.isError()) {
